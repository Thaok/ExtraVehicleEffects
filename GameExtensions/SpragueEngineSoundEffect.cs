...
namespace ExtraVehicleEffects.GameExtensions
{
    ...
    public class SpragueEngineSoundEffect : SoundEffect
    {

        public static SpragueEngineSoundEffect CopyEngineSoundEffect(EngineSoundEffect template, SpragueEngineSoundEffect target)
        {
        ...
        //static private VehicleEffects.Util.PiecewiseLinear m_lowRpmVolumeBySpeed = new VehicleEffects.Util.PiecewiseLinear(new float[,] { { 0.0f, 0.00f }, { m_medium_start, 0.00f }, { m_medium_start + m_fading_width, 0.00f } }, 0.0f);
        //static private VehicleEffects.Util.PiecewiseLinear m_mediumRpmVolumeBySpeed = new VehicleEffects.Util.PiecewiseLinear(new float[,] { { m_medium_start - m_fading_width, 0.00f }, { m_medium_start, 0.00f }, { m_medium_end, 0.00f }, { m_medium_end + m_fading_width + 0.2f, 0.0f } }, 0.0f);
        //static private VehicleEffects.Util.PiecewiseLinear m_highRpmVolumeBySpeed = new VehicleEffects.Util.PiecewiseLinear(new float[,] { { 0.0f, 1.00f }, { m_high_end, 1.00f }, { m_high_end + m_fading_width, 0.00f } }, 0.0f);
        //static private VehicleEffects.Util.PiecewiseLinear m_idleRpmVolumeBySpeed = new VehicleEffects.Util.PiecewiseLinear(new float[,] { { m_high_end - m_fading_width, 0.00f }, { m_high_end, 1.50f }, { m_idle_end, 1.50f } }, 0.0f);
        ...
        private const float m_nonLoopTriggerSpeedTolerance = 0.001f;
        private static readonly float[] m_airstrokeSpeeds = { 0.8f, 0.7f, 0.5f, 0.2f };
        ...
        public AudioInfo m_airstroke_audioInfo;
        ...
        private void addEngineSound(ref List<AudioUtil.AudioInfoPlaybackState> activeAudioInfos, float normalizedSpeed, float magnitude, float volumeAdjustment = 1.0f)
        {
            ...
            if (volume > 0.0f)
            {
                float pitch = 0.95f * m_base_pitch_idle;// (m_pitch_constant + m_pitch_adjust_idle) * normalizedSpeed;
        ...
        public override void PlayEffect(InstanceID id, EffectInfo.SpawnArea area, Vector3 velocity, float acceleration, float magnitude, AudioManager.ListenerInfo listenerInfo, AudioGroup audioGroup)
        {
            ...
            else
            {
                ...
                if(normalizedSpeed < 0.75f)
                    for(uint i=0; i<m_airstrokeSpeeds.Length; ++i)
                        if( Mathf.Abs(normalizedSpeed-m_airstrokeSpeeds[i]) <  m_nonLoopTriggerSpeedTolerance){                        
                            if(i < m_airstrokeSpeeds.Length - 1)
                                activeAudioInfos.Add(new AudioUtil.AudioInfoPlaybackState(this.m_airstroke_audioInfo.m_variations[0].m_sound, 0.5f * magnitude, 1.0f));//just braking
                            else if (i < m_airstrokeSpeeds.Length - 2)
                                activeAudioInfos.Add(new AudioUtil.AudioInfoPlaybackState(this.m_airstroke_audioInfo.m_variations[2].m_sound, 0.5f * magnitude, 1.0f));//platform start
                            else
                                activeAudioInfos.Add(new AudioUtil.AudioInfoPlaybackState(this.m_airstroke_audioInfo.m_variations[1].m_sound, 0.5f * magnitude, 1.0f));//stopping
                        }
                
                                
                //play idle sound during mechanical braking
                {
                   float pitch = 0.95f * m_base_pitch_idle - (1.0f-normalizedSpeed) * 0.2f;
                   activeAudioInfos.Add(new AudioUtil.AudioInfoPlaybackState(this.m_idle_audioInfo, (1.0f - invertedSpeedSqr) * magnitude, pitch));
                }

                //play quieter (no-load) engine sound during mechanical braking
                addEngineSound(ref activeAudioInfos, normalizedSpeed, magnitude, invertedSpeedSqr);

                //brake sqeal before stopping
                if (normalizedSpeed < 1.0f * m_dynamic_brake_cutoff_speed)
                {
